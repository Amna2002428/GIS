...
SITE_URL     = "https://airaqi.ly" # Public domain for QR Code
RANDOM_SEED  = 60
...
def sample_realistic_values(df, random_state=None):
    ...
    # بدلاً من العشوائية المطلقة، نأخذ عينة حقيقية من البيانات للحفاظ على الارتباط بين الملوثات
    idx = rng.integers(0, len(df))
    sample = df.iloc[idx][FEATURES].to_dict()
    # إضافة تذبذب بسيط (jitter) بنسبة 5% لجعل القيم فريدة
    return {k: round(v * rng.uniform(0.95, 1.05), 2) for k, v in sample.items()}

def get_temporal_multiplier():
    """يحاكي تغير التلوث خلال ساعات اليوم (الازدحام المروري)"""
    hour = datetime.datetime.now().hour
    if 7 <= hour <= 20: # ذروة الصباحية (7 صباحاً - 8 مساءً)
        return 1.4
    if hour >= 21 or hour <= 5: # ذروة المسائية (9 مساءً - 5 صباحاً)
        return 0.6
    return 1.0 # بقية اليوم (5-7 صباحاً)

def get_zone_multipliers(device_id):
    """تخصيص بروفايل ملوثات لكل منطقة بناءً على طبيعتها الصناعية والجغرافية"""
    profiles = {
        # تاجوراء: صناعية ثقيلة (PVC، إطارات، غازات طبية، صناعات هندسية، أثاث) + ورشة شاحنات + ساحلية
        "DEV-01": {"all": 1.15, "SO2": 1.4, "CO": 1.3, "PM2.5": 1.25},
        # وادي الربيع: شركة غازات صناعية/طبية + ورش لدائن وأنابيب + مواد بناء وأثاث
        "DEV-02": {"all": 1.1, "NO2": 1.2, "PM10": 1.2},
        # سيدي السايح: تدوير بلاستيك + مطاحن/صوامع حبوب + مصانع بناء (طوب، رخام، جرانيت)
        "DEV-03": {"all": 1.2, "PM10": 1.4, "PM2.5": 1.25, "CO": 1.2},
        # طريق المطار: مستودع طرابلس النفطي (البريقة) + تعبئة غاز + ورش معدات ثقيلة + ازدحام مروري
        "DEV-04": {"all": 1.3, "CO": 1.4, "NO2": 1.3, "SO2": 1.25},
        # جنزور: ساحلية + مصانع بلاستيك وأنابيب (PPR/PVC) + أكياس وعبوات بلاستيكية
        "DEV-05": {"all": 0.9, "PM2.5": 1.15},
        # الكريمية: مصنع إنتاج/تصنيع النفط + مصنع بلاستيك (الأكثر تلوثاً)
        "DEV-06": {"all": 1.4, "SO2": 1.5, "NO2": 1.3, "CO": 1.3},
        # حي الأندلس: سكنية بحتة — لا نشاط صناعي (الأنظف)
        "DEV-07": {"all": 0.7, "O3": 1.1}
    }
    return profiles.get(device_id, {"all": 1.0})
...
for i, loc in enumerate(DEVICE_COORDS, start=1):
    # 1. البدء بعينة من البيانات التاريخية (تحافظ على التوازن الكيميائي)
    s = sample_realistic_values(df, RANDOM_SEED + i)
    
    # 2. تطبيق تأثير الوقت (الازدحام)
    t_mult = get_temporal_multiplier()
    
    # 3. تطبيق بروفايل المنطقة
    z_mults = get_zone_multipliers(loc["device_id"])
    
    # دمج المعاملات
    for k in FEATURES:
        m = t_mult * z_mults.get("all", 1.0) * z_mults.get(k, 1.0)
        s[k] = round(s[k] * m, 2)

    # 4. النطاقات المستهدفة النهائية (لحارس الجودة)
    target_aqi_ranges = {
        "DEV-01": (75, 140),   # صناعية ثقيلة + ساحلية
        "DEV-02": (65, 125),   # صناعية متوسطة
        "DEV-03": (75, 145),   # تدوير + مطاحن + مواد بناء
        "DEV-04": (85, 155),   # مستودع نفطي + مرور
        "DEV-05": (55, 100),   # ساحلية + مصانع خفيفة
        "DEV-06": (95, 170),   # مصنع نفط (الأعلى)
        "DEV-07": (40, 80)     # سكنية بحتة (الأنظف)
    }

    if loc["device_id"] in target_aqi_ranges:
        min_aqi, max_aqi = target_aqi_ranges[loc["device_id"]]
        # خوارزمية البحث ثنائي لضبط القيم لتناسب النطاق الجغرافي المطلوب
        low, high = 0.05, 3.0
        best_s = s.copy()
        best_p = 0
        for _ in range(20):
            ...
            best_s = test_s
            best_p = p
            if min_aqi <= p <= max_aqi: break
            elif p > max_aqi: high = mid
            else: low = mid
        s = best_s
        p = round(best_p, 2)
    else:
...
@media (max-width: 1024px) {
  ...
  .nav-logo span { display: none; } /* Hide long text on mobile */
  .nav-logo::after { content: 'AQI Dashboard'; font-size: 12px; opacity: 0.9; }
}
.nav-logo {
...
@media (max-width: 600px) {
  ...
  .nav-btn span { display: none; } /* Hide text on small screens, keep icons/emojis */
  .nav-right { gap: 5px; }
}
@media (max-width: 380px) {
  .nav-btn { padding: 6px 8px; }
  .nav-logo-icon { display: none; } /* Save more space on extreme small screens */
}
...
@media (max-width: 1024px) {
  #detail-sidebar {
    width: 100%;
    left: -100%;
    height: calc(100dvh - var(--nav-height) - 55px);
...
.sb-aqi-display { text-align: center; margin: 15px 0 5px; }
.sb-aqi-value {
  font-size: 82px; font-weight: 300; line-height: 1;
  color: #1a2236; letter-spacing: -3px;
}
@media (max-width: 768px) {
  .sb-aqi-value { font-size: 64px; }
}
.sb-aqi-caption { 
  font-size: 13px; color: #9ca3b0; margin-top: 4px; 
  display: flex; align-items: center; justify-content: center; gap: 5px;
}
.sb-aqi-caption::after { content: 'ⓘ'; font-size: 14px; opacity: 0.6; cursor: help; }
...
#map {
  position: fixed;
  top: var(--nav-height); bottom: 0; left: 0; right: 0;
  z-index: 100;
}
@media (max-width: 1024px) {
  #map { bottom: 50px; }
...
@media (max-width: 1024px) {
  #legend { 
    bottom: 110px; left: 10px; right: 10px; transform: translateY(20px); 
    margin-left: 0; flex-wrap: wrap; justify-content: center; gap: 6px;
    padding: 12px; font-size: 10px;
    max-height: 150px; overflow-y: auto;
    border: 1px solid rgba(255,255,255,0.1);
    opacity: 0; visibility: hidden; pointer-events: none;
    background: rgba(27,34,50,0.98);
...
#legend-toggle {
  ...
  background: rgba(27,34,50,.92); color: #fff;
  border: 1px solid rgba(255,255,255,0.1);
...
body.light-theme #legend-toggle {
  background: #fff; color: #1a2236; border-color: #eee;
}
...
<nav id="navbar">
  ...
  <div class="nav-right">
    <button class="nav-btn" id="lang-btn" onclick="toggleLanguage()">🌐 <span>English</span></button>
...
<!-- Mobile Toggle Legend Button -->
<button id="legend-toggle" title="دليل الألوان">ⓘ</button>
...
var SITE_URL = "{site_url}"; // Fixed public URL
...
function updateI18nTexts() {
  ...
  document.getElementById('share-btn').innerHTML = '🔗 <span data-i18n="share">' + I18N[currentLang].share + '</span>';
  document.getElementById('theme-btn').innerHTML = (isLightTheme ? '🌙 ' : '☀️ ') + '<span data-i18n="' + (isLightTheme ? 'dark' : 'light') + '">' + (isLightTheme ? I18N[currentLang].dark : I18N[currentLang].light) + '</span>';
  document.getElementById('lang-btn').innerHTML = '🌐 <span>' + (currentLang === 'ar' ? 'English' : 'العربية') + '</span>';
    ...
    if (layer.getLatLng && layer.options && layer.options.radius === 16) {
      ...
      if (dev) {
         ...
         layer.setTooltipContent(
           ...
           'AQI: <b>' + Math.round(dev.predicted_AQI) + '</b><br>' +
...
function toggleQR() {
  ...
  if (modal.classList.contains('open') && !qrGenerated) {
    ...
    // If we're on a local server or file, use current URL (for local network testing)
    // If we're on the public domain, use the official SITE_URL
    var qrLink = SITE_URL;
    ...
    if (isLocal || !qrLink || qrLink.includes('{{')) {
...
panelToggle.addEventListener('click', function() {
  ...
  // Close legend if panel opens
  if (isOpen) {
    legend.classList.remove('open');
    legendToggle.style.background = '';
    legendToggle.style.color = '';
  }
...
legendToggle.addEventListener('click', function() {
  ...
  if (isOpen) {
    // Close device panel if legend opens on mobile
    devicePanel.classList.remove('open');
    panelToggle.innerText = '☰';
    legendToggle.style.background = '#22d3ee';
    legendToggle.style.color = '#fff';
  } else {
    legendToggle.style.background = '';
    legendToggle.style.color = '';
  }
...
function updateTabUIContent() {
  ...
  DEVICES.forEach(function(dev, idx) {
    if (dev._bat === undefined) dev._bat = (idx === 0) ? 5 : 100 - (dev.name.length % 20);

    var currentHour = new Date().getHours();
    dev._isCharging = (currentHour >= 8 && currentHour < 18);

    if (dev._isOnline === undefined) {
       if (dev._isCharging && dev._bat < 15) {
           dev._isOnline = false;
       } else if (!dev._isCharging && dev._bat <= 0) {
           dev._isOnline = false;
       } else {
           dev._isOnline = Math.random() > 0.15;
       }
    }
    ...
    var batIcon = dev._isCharging ? '⚡️' : (dev._bat > 20 ? '🔋' : '🪫');
    ...
    sHTML += '<div class="sensor-card">' +
      ...
      '<div class="sensor-row"><span>' + I18N[currentLang].battery + ':</span><span>' + dev._bat + '% ' + batIcon + '</span></div>' +
  ...
  DEVICES.forEach(function(dev) {
    if(dev.predicted_AQI > 100) {
      alertsHTML += '<div class="alert-item">' +
        ...
        '<div class="alert-msg">' + I18N[currentLang].alert_warning + ' <span style="font-weight:bold;color:' + getColor(dev.predicted_AQI) + '">' + getLabel(dev.predicted_AQI) + '</span> ' + I18N[currentLang].alert_station + ' ' + dev.name + '. ' + I18N[currentLang].alert_index + ': ' + Math.round(dev.predicted_AQI) + '</div>' +
      '</div>';
      hasAlert = true;
    }
  });
...
function openSidebar(dev, idx) {
  ...
  if (document.documentElement.dir === 'ltr') arrowPct = pct.toFixed(1) + '%';
...
setInterval(function() {
  ...
  DEVICES.forEach(function(dev, idx) {
    if (dev._bat === undefined) {
       dev._bat = (idx === 0) ? 5 : 100 - (dev.name.length % 20);
    }
    ...
    if (dev._isCharging) {
        // Charging mode
        if (dev._bat < 100) {
            var chargeProb = (currentHour < 16) ? (1.0 / 18.0) : (1.0 / 72.0); // Morning vs Sunset
            
            // Trickle charge (slower) when battery is critically low (< 15%)
            if (dev._bat < 15) {
                chargeProb = chargeProb / 2.0; 
            }
        ...
        // Enforce state while charging
        if (dev._bat < 15) {
            dev._isOnline = false;
        } else {
            dev._isOnline = true; // Auto recovery when charge reaches 15%
        }
    } else {
        // Discharging mode
        if (dev._bat > 0) {
            // e.g. 1% every 10 minutes (60 ticks)
    ...
    // Random network drops (independent of battery, only if online)
    if (dev._isOnline) {
      if (Math.random() < 0.02) {
          dev._isOnline = false;
      }
    } else if (dev._bat >= 15 || (!dev._isCharging && dev._bat > 0)) {
      // Recovery chance if offline due to network (not battery)
      if (Math.random() < 0.05) {
          dev._isOnline = true;
      }
    }
    ...
    if (dev._isOnline) {
      ...
      ['PM2.5','PM10','NO2','SO2','CO','O3'].forEach(function(f) {
        if (dev[f] !== undefined) {
           ...
           // Crucial: Round the data itself to prevent floating point accumulation and display issues
           dev[f] = Math.round(dev[f] * 100) / 100;
        }
      });
    ...
    if (card) {
      ...
      card.querySelector('.dev-circle').innerText = Math.round(dev.predicted_AQI);
      card.querySelector('.dev-sub').innerText = dev.device_id + ' | AQI ' + Math.round(dev.predicted_AQI);
  ...
  if (activeCard) {
    ...
    if (sBody) {
       ...
       if (sbAqi) { sbAqi.innerText = Math.round(openDev.predicted_AQI); }
  ...
  DEVICES.forEach(function(dev) {
    ...
    sHTML += '<div class="sensor-card">' +
      '<div class="s-card-title">' + dev.name + ' <span style="font-size:11px;color:#9aa5be;font-weight:400">(' + dev.device_id + ')</span></div>' +
      '<div class="sensor-row"><span>' + I18N[currentLang].online.split(' ')[0] + ':</span><span style="font-weight:bold">' + isOnline + '</span></div>' +
      '<div class="sensor-row"><span>' + I18N[currentLang].battery + ':</span><span><span style="display:inline-block;width:30px;">' + dev._bat + '%</span> ' + batIcon + '</span></div>';
    
    if (dev._isOnline) {
      sHTML += '<div class="sensor-row"><span>' + I18N[currentLang].last_read + ':</span><span>' + dev._lastUpdate + '</span></div>';
    } else {
      sHTML += '<div class="sensor-row"><span>' + I18N[currentLang].last_read + ':</span><span style="color:#ef4444">' + (dev._lastUpdate || '---') + '</span></div>';
    }
    sHTML += '</div>';
  });
  var sContainer = document.getElementById('sensors-content');
...
with open(MAP_PATH, 'w', encoding='utf-8') as f:
    f.write(html.replace('{devices_json}', devices_json)
