...
MAP_PATH     = "aqi_heatmap_dashboard.html"
...
<nav id="navbar">
  <div class="nav-links">
    <div class="nav-link active" id="nav-main" onclick="switchTab('main')">الرئيسية (الخريطة)</div>
    <div class="nav-link" id="nav-sensors" onclick="switchTab('sensors')">أجهزة الاستشعار</div>
    <div class="nav-link" id="nav-readings" onclick="switchTab('readings')">القراءات</div>
    <div class="nav-link" id="nav-alerts" onclick="switchTab('alerts')">التنبيهات</div>
  </div>
  <div class="nav-logo">
    <div class="nav-logo-icon">AQ</div>
    نظام مراقبة جودة الهواء اللحظي 
  </div>
  <div class="nav-right">
    <button class="nav-btn" id="theme-btn">🌙 داكن</button>
  </div>
</nav>

<!-- RIGHT DEVICE PANEL -->
<aside id="device-panel">
  <div class="panel-heading">محطات المراقبة</div>
...
<!-- LEGEND -->
<div id="legend">
  <div class="legend-label"><div class="legend-dot" style="background:#8cc63f"></div> جيد (0–50)</div>
  <div class="legend-label"><div class="legend-dot" style="background:#fdb913"></div> معتدل (51–100)</div>
  <div class="legend-label"><div class="legend-dot" style="background:#f37021"></div> حساس (101–150)</div>
  <div class="legend-label"><div class="legend-dot" style="background:#ed1c24"></div> غير صحي (151–200)</div>
  <div class="legend-label"><div class="legend-dot" style="background:#9e005d"></div> خطير (201–300)</div>
  <div class="legend-label"><div class="legend-dot" style="background:#790000"></div> طارئ (300+)</div>
</div>

<!-- TAB VIEWS (NOT MAP) -->
<div id="view-sensors" class="tab-view hidden">
  <div class="view-header">حالة الأجهزة والمستشعرات</div>
  <div class="sensors-grid" id="sensors-content"></div>
</div>

<div id="view-readings" class="tab-view hidden">
  <div class="view-header">سجل القراءات الحية</div>
  <div class="table-wrapper">
    <table class="readings-table">
       <thead>
          <tr><th>المحطة</th><th>AQI</th><th>PM2.5</th><th>PM10</th><th>NO2</th><th>SO2</th><th>CO</th><th>O3</th></tr>
       </thead>
...
<div id="view-alerts" class="tab-view hidden">
  <div class="view-header">التنبيهات النظامية</div>
...
function arabicLabel(cat) {{
  var m = {{
    'Good': 'جيد',
    'Moderate': 'معتدل',
    'Unhealthy for Sensitive Groups': 'غير صحي للحساسين',
    'Unhealthy': 'غير صحي',
    'Very Unhealthy': 'غير صحي جداً',
    'Hazardous': 'خطير'
  }};
  return m[cat] || cat;
}}

function arabicDesc(cat) {{
  var d = {{
    'Good': 'الهواء نظيف تماماً. يمكنك الاستمتاع بجميع الأنشطة الخارجية بكل راحة.',
    'Moderate': 'جودة الهواء مقبولة بشكل عام، لكن قد يشعر بعض الحساسين ببعض القلق.',
    'Unhealthy for Sensitive Groups': 'قد يعاني الأشخاص ذوو الأمراض التنفسية وكبار السن من أعراض صحية.',
    'Unhealthy': 'قد يشعر الجميع بآثار صحية؛ والمجموعات الحساسة معرضة لمخاطر أكبر.',
    'Very Unhealthy': 'تحذير صحي! من المرجح أن يتأثر غالبية السكان بشكل ملحوظ.',
    'Hazardous': 'طوارئ صحية: تأثيرات خطيرة على جميع الفئات دون استثناء.'
  }};
  return d[cat] || '';
}}
...
themeBtn.addEventListener('click', function() {
  ...
  if(isLightTheme) {
    ...
    themeBtn.innerText = '🌙 داكن';
  } else {
    document.body.classList.remove('light-theme');
    themeBtn.innerText = '☀️ مضيء';
  }
});
...
function updateTabUIContent() {{
  ...
  DEVICES.forEach(function(dev) {{
    ...
    var isOnline = dev._isOnline ? 'متصل 🟢' : 'غير متصل 🔴';
    sHTML += '<div class="sensor-card">' +
      ...
      '<div class="sensor-row"><span>حالة الاتصال:</span><span style="font-weight:bold">' + isOnline + '</span></div>' +
      '<div class="sensor-row"><span>مستوى البطارية:</span><span>' + dev._bat + '% 🔋</span></div>' +
      '<div class="sensor-row"><span>وقت آخر قراءة:</span><span>' + dev._lastUpdate + '</span></div>' +
    '</div>';
  }});
  ...
  DEVICES.forEach(function(dev) {{
    if(dev.predicted_AQI > 100) {{
      alertsHTML += '<div class="alert-item">' +
        ...
        '<div class="alert-msg">تحذير: هواء <span style="font-weight:bold;color:' + getColor(dev.predicted_AQI) + '">' + arabicLabel(dev.AQI_category) + '</span> في ' + dev.name + '. المؤشر: ' + dev.predicted_AQI + '</div>' +
      '</div>';
      hasAlert = true;
    }}
  }});
  if(!hasAlert) {{
    alertsHTML = '<div class="alert-item" style="border-right-color:#8cc63f">' +
      '<div class="alert-time">' + dStr.split(' - ')[1] + '</div>' +
      '<div class="alert-msg" style="color:#8cc63f">جميع المحطات تعمل بشكل ممتاز والمؤشرات ضمن المعدلات الطبيعية.</div>' +
    '</div>';
  }}
...
DEVICES.forEach(function(dev, idx) {{
  ...
  card.innerHTML =
    ...
    '<div class="dev-body">' +
      ...
      '<div class="dev-sub">' + dev.device_id + ' | AQI ' + dev.predicted_AQI + '</div>' +
      '<span class="dev-badge" style="background:' + c + ';color:' + tc + '">' +
        arabicLabel(dev.AQI_category) +
      '</span>' +
    '</div>';
  ...
  marker.bindTooltip(
    ...
    '<b>' + dev.name + '</b><br>' +
    '<div style="font-size:11px;opacity:0.8;margin-bottom:2px;">الجهاز: ' + dev.device_id + '</div>' +
    'AQI: <b>' + dev.predicted_AQI + '</b><br>' +
    '<span style="color:' + c + '">' + arabicLabel(dev.AQI_category) + '</span>' +
    '</div>',
  ...
  sbBody.innerHTML =
    '<div class="sb-card">' +
      '<div class="sb-top-row">' +
        ...
        '<div class="sb-dev-num">رقم الجهاز : ' + dev.device_id + '</div>' +
      '</div>' +
      ...
      '<div class="sb-aqi-display">' +
        '<div class="sb-aqi-value">' + dev.predicted_AQI + '</div>' +
        '<div class="sb-aqi-caption">مؤشر جودة الهواء</div>' +
      '</div>' +
    ...
    '<div class="info-box">' +
      '<div class="info-icon" style="color:' + c + '">&#9432;</div>' +
      '<div>' +
        '<div class="info-heading">' + arabicLabel(dev.AQI_category) + '</div>' +
        '<div class="info-body">' + arabicDesc(dev.AQI_category) + '</div>' +
      '</div>' +
    '</div>' +
...
setInterval(function() {{
  ...
  DEVICES.forEach(function(dev, idx) {{
    ...
    if (card) {{
      ...
      badge.innerText = arabicLabel(dev.AQI_category);
    }}
  }});
  ...
  if (activeCard) {{
    ...
    if (sBody) {{
       ...
       if (sbStatus) {{ sbStatus.style.background = c; sbStatus.style.color = tc; sbStatus.innerText = arabicLabel(openDev.AQI_category); }}
       ...
       if (infHead) {{ infHead.innerText = arabicLabel(openDev.AQI_category); }}
       var infBody = sBody.querySelector('.info-body');
       if (infBody) {{ infBody.innerText = arabicDesc(openDev.AQI_category); }}
  ...
  DEVICES.forEach(function(dev) {{
    ...
    sHTML += '<div class="sensor-card">' +
      '<div class="s-card-title">' + dev.name + ' <span style="font-size:11px;color:#9aa5be;font-weight:400">(' + dev.device_id + ')</span></div>' +
      '<div class="sensor-row"><span>حالة الاتصال:</span><span style="font-weight:bold">' + isOnline + '</span></div>' +
      '<div class="sensor-row"><span>مستوى البطارية المستشعر:</span><span>' + dev._bat + '% 🔋</span></div>' +
      '<div class="sensor-row"><span>وقت آخر قراءة:</span><span>' + dev._lastUpdate + '</span></div>' +
    '</div>';
  }});
  var sContainer = document.getElementById('sensors-content');
  ...
  DEVICES.forEach(function(dev) {{
    if(dev.predicted_AQI > 100) {{
      alertsHTML += '<div class="alert-item">' +
        '<div class="alert-time">' + dStr.split(' - ')[1] + '</div>' +
        '<div class="alert-msg">تحذير تنبيه: تم تسجيل هواء <span style="font-weight:bold;color:' + getColor(dev.predicted_AQI) + '">' + arabicLabel(dev.AQI_category) + '</span> في المحطة ' + dev.name + '. المؤشر: ' + dev.predicted_AQI + '</div>' +
      '</div>';
      hasAlert = true;
    }}
  }});
