...
def get_zone_multipliers(device_id):
    """تخصيص بروفايل ملوثات لكل منطقة بناءً على طبيعتها"""
    profiles = {
        "DEV-01": {"all": 0.9, "SO2": 1.2},        # تاجوراء: صناعية خفيفة/ساحلية
        "DEV-02": {"all": 1.0},                    # وادي الربيع: ريفية/سكنية
        "DEV-03": {"all": 1.1, "PM10": 1.3},       # سيدي السايح: مفتوحة/أتربة
        "DEV-04": {"all": 1.2, "CO": 1.4, "NO2": 1.3}, # طريق المطار: ازدحام مروري
        "DEV-05": {"all": 0.75, "O3": 1.1},        # جنزور: سكنية ساحلية (هواء أنقى)
        "DEV-06": {"all": 1.3, "SO2": 1.4, "CO": 1.2}, # الكريمية: صناعية/تجارية كثيفة
        "DEV-07": {"all": 0.7, "O3": 1.1}          # حي الاندلس: سكنية ساحلية (هواء أنقى)
    }
...
#legend {
  ...
  transition: margin-left var(--transition-speed) ease, opacity 0.3s ease, transform 0.3s ease;
}
@media (max-width: 1024px) {
  #legend { 
...
panelToggle.addEventListener('click', function() {
  ...
  // On desktop, adjust map right padding for the panel
